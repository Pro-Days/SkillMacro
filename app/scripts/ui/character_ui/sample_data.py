...
STAT_DIST: tuple[tuple[str, str, str, int], ...] = (
    ("strength", "힘", "공격력 +1\n치명타 공격력% +0.1", 200),
    ("dexterity", "민첩", "공격력% +0.3\n치명타 확률% +0.05", 180),
    ("vitality", "생명력", "체력 +5, 회피% +0.03\n물약 회복량% +0.5", 160),
    ("luck", "행운", "경험치 획득량% +0.2\n드랍률% +0.2", 200),
)

# 단전 분배 3종 (키, 라벨, 효과 설명)
DANJEON_DIST: tuple[tuple[str, str, str, int], ...] = (
    ("upper", "상단전", "치명타 확률% +1 · 치명타 공격력% +1", 6),
    ("middle", "중단전", "공격력% +1", 5),
    ("lower", "하단전", "드랍률% +1.5 · 경험치 획득량% +0.5", 26),
)
...
SCROLL_SETS: dict[str, ScrollSet] = {
    "armorH": ScrollSet(
        ("힘", "민첩", "생명력", "행운", "행운%"), ("10%", "20%", "50%", "60%", "100%")
    ),
    "armorA": ScrollSet(
        ("힘", "민첩", "생명력", "행운", "힘%"), ("10%", "20%", "50%", "60%", "100%")
    ),
    "armorB": ScrollSet(
        ("힘", "민첩", "생명력", "행운", "생명력%"),
        ("10%", "20%", "50%", "60%", "100%"),
    ...
    "armorS": ScrollSet(
        ("힘", "민첩", "생명력", "행운", "민첩%"), ("10%", "20%", "50%", "60%", "100%")
    ),
    "weapon": ScrollSet(
        ("공격력", "힘", "민첩", "공격력%"), ("10%", "20%", "40%", "50%", "60%", "100%")
    ),
    "ring": ScrollSet(
        ("힘", "민첩", "생명력", "행운"), ("10%", "20%", "50%", "60%", "100%")
    ),
}
...
# 슬롯에 보유한 장비 1개 (유저가 만든 장비 인스턴스)
@dataclass
class EquipItemData:
    name: str
    level: int
    grade: str = "기본"  # 기본 / 찬란한 (무기·방어구만 의미)
    tier: int = 1  # 장비 티어 (1~5)
    base: list[tuple[str, float]] = field(
        default_factory=list
    )  # 방어구/무기/목걸이 기본 스탯
    free_base: list[tuple[str, str]] = field(
        default_factory=list
    )  # 반지/귀걸이 자유 기본 스탯 (스탯, 값)
    necklace_type: str | None = None  # 목걸이 종류 (목걸이만)
    reforge_step: int = 0  # 재련 단계 (선택 화면 이름 옆 표시)
    scroll_stat: list[str] = field(
        default_factory=list
    )  # 주문서 총 스탯 요약 (선택 화면 표시)
    potential: list[str] = field(default_factory=list)  # 잠재능력 요약 (선택 화면)
    additional: list[str] = field(default_factory=list)  # 추가능력 요약 (선택 화면)
...
class EquipSlotData:
    key: str
    type: str  # 부위명
    reforge: bool  # 재련 가능 여부 (부위 고정)
    scroll: str | None  # 주문서 세트 키 (부위 고정)
    owned: list[EquipItemData]  # 슬롯에 보유한 장비 목록
    equipped_index: int = 0  # 장착된 장비 인덱스 (-1 이면 미장착)

    def equipped(self) -> EquipItemData | None:
        """현재 장착된 장비 (미장착이면 None)"""

        if 0 <= self.equipped_index < len(self.owned):
...
def default_equip_slots() -> list[EquipSlotData]:
    ...
    슬롯마다 유저가 만든 장비 여러 개를 미리 채워 장비 교체(선택) 화면을 보여준다.
    """
        ...
        EquipSlotData(
            ...
            [
                EquipItemData(
                    ...
                    reforge_step=12,
                    scroll_stat=["행운 +18", "행운% +9", "체력% +3"],
                    potential=["행운% +3.5", "행운% +3.3"],
                    additional=["체력 +23", "행운 +7.7"],
                ),
                EquipItemData(
                    "산령투구",
                    150,
                    "찬란한",
                    1,
                    [("행운", 48), ("행운%", 20), ("공격력", 13), ("체력", 120), ("체력%", 2)],
                    reforge_step=15,
                    scroll_stat=["행운 +24", "행운% +12", "체력% +4"],
                    potential=["행운% +4.0", "행운% +3.8", "치명타 공격력% +2.1"],
                    additional=["체력 +41", "행운 +9.2", "치명타 확률% +1.9"],
                ),
            ],
        ),
        EquipSlotData(
            ...
            [
                EquipItemData(
                    ...
                    reforge_step=11,
                    scroll_stat=["힘 +16", "힘% +8", "체력% +3"],
                    potential=["힘% +3.6", "체력% +6.0"],
                    additional=["힘 +8", "체력 +30"],
                ),
                EquipItemData(
                    "투령갑옷",
                    150,
                    "찬란한",
                    3,
                    [("힘", 44), ("힘%", 16), ("공격력", 12), ("체력", 110), ("체력%", 2)],
                    reforge_step=9,
                    scroll_stat=["힘 +12", "힘% +6", "체력% +2"],
                    potential=["힘% +3.9", "힘% +3.5", "보스 공격력% +4.2"],
                    additional=["힘 +9.5", "체력 +44"],
                ),
            ],
        ),
        EquipSlotData(
            ...
            [
                EquipItemData(
                    ...
                    reforge_step=10,
                    scroll_stat=["생명력 +15", "생명력% +7", "체력% +3"],
                    potential=["생명력% +3.4", "체력% +5.5"],
                    additional=["생명력 +7", "체력 +28"],
                ),
            ],
        ),
        EquipSlotData(
            ...
            [
                EquipItemData(
                    ...
                    reforge_step=9,
                    scroll_stat=["민첩 +14", "민첩% +6", "체력% +3"],
                    potential=["민첩% +3.2", "민첩% +3.0"],
                    additional=["민첩 +6", "체력 +25"],
                ),
            ],
        ),
        EquipSlotData(
            ...
            [
                EquipItemData(
                    ...
                    "기본",
                    5,
                    [("공격력", 160), ("최종 공격력%", 4), ("힘", 30), ("민첩", 22)],
                    reforge_step=15,
                    scroll_stat=["공격력 +21", "민첩 +6", "치명타 확률% +2"],
                ),
                EquipItemData(
                    "이매귀도",
                    150,
                    "찬란한",
                    1,
                    [("공격력", 193), ("최종 공격력%", 4.8), ("스킬 피해량%", 3.1), ("치명타 확률%", 1)],
                    reforge_step=13,
                    scroll_stat=["공격력 +18", "민첩 +3", "치명타 확률% +1.5"],
                ),
                EquipItemData(
                    "화염마검",
                    180,
                    "기본",
                    1,
                    [("공격력", 222), ("최종 공격력%", 5.8), ("힘", 34), ("민첩", 26)],
                    reforge_step=8,
                    scroll_stat=["공격력 +12", "힘 +9"],
                ),
            ],
        ),
        EquipSlotData(
            ...
            [
                EquipItemData(
                    "황룡반지",
                    160,
                    free_base=[("행운", "24"), ("체력", "70"), ("행운%", "5")],
                    scroll_stat=["행운 +9", "체력% +2"],
                ),
                EquipItemData(
                    "주작반지",
                    160,
                    free_base=[("생명력", "10"), ("생명력%", "5"), ("체력%", "4")],
                    scroll_stat=["생명력 +9", "생명력% +5"],
                ),
            ],
        ),
        EquipSlotData(
            ...
            [
                EquipItemData(
                    "청룡반지",
                    120,
                    free_base=[("힘", "24"), ("체력", "70"), ("힘%", "5")],
                    scroll_stat=["힘 +12", "체력% +4"],
                ),
            ],
        ),
        EquipSlotData(
            ...
            [
                EquipItemData(
                    "적옥목걸이",
                    100,
                    base=[("공격력", 80), ("치명타 공격력%", 14), ("보스 공격력%", 8)],
                    necklace_type="적옥목걸이",
                    reforge_step=7,
                ),
                EquipItemData(
        ...
        EquipSlotData(
            ...
            [
                EquipItemData(
                    "금귀걸이",
                    110,
                    free_base=[("공격력", "72"), ("스킬 피해량%", "10"), ("최종 공격력%", "5")],
                ),
            ],
        ),
    ]
