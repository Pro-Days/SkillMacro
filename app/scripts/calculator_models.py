...
class OwnedTalisman:
    ...
    def from_dict(cls, data: dict[str, int | str]) -> "OwnedTalisman":
        ...
        owned_id: str = str(data["owned_id"])
        template_id: str = str(data["template_id"])
        level: int = int(data["level"])
        return cls(owned_id=owned_id, template_id=template_id, level=level)

    def to_dict(self) -> dict[str, int | str]:
        """보유 부적 직렬화"""

        data: dict[str, int | str] = {
            "owned_id": self.owned_id,
...
@dataclass(slots=True)
class EquippedOptimizationState:
    """현재 장착 선택 상태"""

    equipped_title_id: str | None = None
    equipped_talisman_ids: list[str] = field(default_factory=list)

    @classmethod
    def from_dict(cls, data: dict[str, object]) -> "EquippedOptimizationState":
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        data: dict[str, object] = {
            ...
            "equipped_talisman_ids": self.equipped_talisman_ids.copy(),
        }
        return data
...
# 계산기 스탯 표시 스펙 고정
CALCULATOR_STAT_SPECS: dict[StatKey, CalculatorStatSpec] = {
    StatKey.ATTACK: CalculatorStatSpec(label="공격력", is_percent=False),
    StatKey.ATTACK_PERCENT: CalculatorStatSpec(label="공격력", is_percent=True),
    StatKey.HP: CalculatorStatSpec(label="체력", is_percent=False),
    StatKey.HP_PERCENT: CalculatorStatSpec(label="체력", is_percent=True),
    StatKey.STR: CalculatorStatSpec(label="힘", is_percent=False),
    StatKey.STR_PERCENT: CalculatorStatSpec(label="힘", is_percent=True),
    StatKey.DEXTERITY: CalculatorStatSpec(label="민첩", is_percent=False),
    StatKey.DEXTERITY_PERCENT: CalculatorStatSpec(label="민첩", is_percent=True),
    StatKey.VITALITY: CalculatorStatSpec(label="생명력", is_percent=False),
    StatKey.VITALITY_PERCENT: CalculatorStatSpec(label="생명력", is_percent=True),
    StatKey.LUCK: CalculatorStatSpec(label="행운", is_percent=False),
    StatKey.LUCK_PERCENT: CalculatorStatSpec(label="행운", is_percent=True),
    StatKey.SKILL_DAMAGE_PERCENT: CalculatorStatSpec(
        label="스킬 피해량", is_percent=True
    ),
    StatKey.FINAL_ATTACK_PERCENT: CalculatorStatSpec(
        label="최종 공격력", is_percent=True
    ),
    StatKey.CRIT_RATE_PERCENT: CalculatorStatSpec(label="치명타 확률", is_percent=True),
    StatKey.CRIT_DAMAGE_PERCENT: CalculatorStatSpec(
        label="치명타 공격력", is_percent=True
    ),
    StatKey.EXP_PERCENT: CalculatorStatSpec(label="경험치 획득량", is_percent=True),
    StatKey.BOSS_ATTACK_PERCENT: CalculatorStatSpec(
        label="보스 공격력", is_percent=True
    ),
    StatKey.DROP_RATE_PERCENT: CalculatorStatSpec(label="드랍률", is_percent=True),
    StatKey.DODGE_PERCENT: CalculatorStatSpec(label="회피", is_percent=True),
    StatKey.POTION_HEAL_PERCENT: CalculatorStatSpec(
        label="물약 회복량", is_percent=True
    ),
    StatKey.RESIST_PERCENT: CalculatorStatSpec(label="저항", is_percent=True),
    StatKey.SKILL_SPEED_PERCENT: CalculatorStatSpec(label="스킬속도", is_percent=True),
}
...
REALM_TIER_SPECS: dict[RealmTier, RealmTierSpec] = {
    ...
    RealmTier.SECOND_RATE: RealmTierSpec(
        label="이류",
        min_level=30,
        danjeon_points=2,
    ),
    RealmTier.FIRST_RATE: RealmTierSpec(
        label="일류",
        min_level=60,
        danjeon_points=5,
    ),
    RealmTier.PEAK: RealmTierSpec(
        label="절정",
        min_level=90,
        danjeon_points=10,
    ),
    RealmTier.TRANSCENDENT: RealmTierSpec(
        label="초절정",
        min_level=120,
        danjeon_points=17,
    ),
    RealmTier.HWAGYEONG: RealmTierSpec(
        label="화경",
        min_level=150,
        danjeon_points=26,
    ),
    RealmTier.HYEONGYEONG: RealmTierSpec(
        label="현경",
        min_level=180,
        danjeon_points=37,
    ),
    RealmTier.LIFE_AND_DEATH: RealmTierSpec(
        label="생사경",
        min_level=200,
        danjeon_points=50,
    ),
...
# 부적 등급 보정값 고정
TALISMAN_GRADE_OFFSETS: dict[TalismanGrade, int] = {
    TalismanGrade.ADVANCED: 0,
    TalismanGrade.RARE: 1,
    TalismanGrade.HERO: 2,
    TalismanGrade.LEGEND: 3,
}
