...
def check_kb_pressed(shared_data):
    while True:
        key = kb.read_key()
        ...
        if convertedKey == shared_data.startKey:
            if shared_data.is_activated:  # On -> Off
                ...
                Thread(target=runMacro, args=[shared_data, shared_data.loop_num]).start()
        ...
        # 연계스킬 사용
            ...
            for i in range(len(linkKeys)):
                if convertedKey == linkKeys[i]:
                    skills = [
                        shared_data.linkSkillList[i]["skills"][num][0]
                        for num in range(len(shared_data.linkSkillList[i]["skills"]))
                    ]

                    canStart = True
                    for skill in skills:
                        if not skill in shared_data.selectedSkillList:
                            canStart = False
                            break

                    if canStart:
                        Thread(target=useLinkSkill, args=[shared_data, i, shared_data.loop_num]).start()

            time.sleep(0.25 * shared_data.SLEEP_COEFFICIENT_NORMAL)


def runMacro(shared_data, loop_num):
    """
    매크로 메인 쓰레드
    """
    ...
    shared_data.skillCoolTimers = [None] * shared_data.USABLE_SKILL_COUNT[shared_data.serverID]
    ...
    # 스킬 쿨타임 쓰레드
    Thread(target=updateSkillCooltime, args=[shared_data, loop_num], daemon=True).start()
    ...
    while shared_data.is_activated and shared_data.loop_num == loop_num:  # 매크로 작동중일 때
        addTaskList(shared_data)  # taskList에 사용 가능한 스킬 추가
        usedSkill = useSkill(shared_data, loop_num)  # 스킬 사용하고 시간, 사용한 스킬 리턴. skill: slot
        ...
        #         shared_data.selectedSkillList[i]
        #     ]} : {shared_data.skillCoolTimers[i]} / {int(shared_data.SKILL_COOLTIME_LIST[shared_data.serverID][shared_data.jobID][shared_data.selectedSkillList[i]] * (100 - shared_data.cooltimeReduce))}"
        #     )
        # print()
    # print(usedSkillList)


def macroClick(shared_data, loopNum):
...
def updateSkillCooltime(shared_data, loopNum):
...
def initMacro(shared_data):
    ...
    shared_data.preparedSkillList = [  # 0~5
        [
            i
            for i in range(6)
            if not shared_data.ifUseSkill[shared_data.selectedSkillList[i]]
            and shared_data.selectedSkillList[i] != -1
        ],
        [
            i
            for i in range(6)
            if shared_data.ifUseSkill[shared_data.selectedSkillList[i]]
            and shared_data.selectedSkillList[i] != -1
        ],
        [],  # append 대기
    ]
    ...
    shared_data.preparedSkillComboList = [
        shared_data.comboCount[shared_data.selectedSkillList[i]] for i in range(6)
    ]  # 0~5

    # 개별 우선순위 -> 등록 순서
    shared_data.skillSequences = []  # 0~5 in self.selectedSkillList
    ...
    for i in range(1, 7):
        for j, k in enumerate(shared_data.skillPriority):  # 0~7, 0~5
            if k == i:
    ...
    # print(self.skillSequences)
    for i in range(6):
        if not (i in shared_data.skillSequences) and i in shared_data.preparedSkillList[1]:
            shared_data.skillSequences.append(i)

    shared_data.usingLinkSkillList = []
    shared_data.usingLinkSkillComboList = []
    for i in shared_data.linkSkillList:
        if i["useType"] == 0:
            shared_data.usingLinkSkillList.append([])
            shared_data.usingLinkSkillComboList.append([])
            for j in i["skills"]:
                x = convert7to5(shared_data, j[0])
                # if not (x in self.requireLinkSkillList[-1]):
                shared_data.usingLinkSkillList[-1].append(x)
    ...
    for i, j in enumerate(shared_data.usingLinkSkillList):
    ...
    shared_data.preparedLinkSkillList = list(range(len(shared_data.usingLinkSkillList)))

    shared_data.taskList = []
    # self.printMacroInfo(brief=False)


def useSkill(shared_data, loopNum):
    ...
    def use(skill):
        if (
            ...
            shared_data.skillCoolTimers[skill] = time.time()  # 스킬 스택이 모두 찬 상태일 때

        shared_data.availableSkillCount[skill] -= 1

    if len(shared_data.taskList) != 0:
...
def useLinkSkill(shared_data, num, loopNum):
    def press(key):
    ...
    slot = -1
    taskList = []
    for i in shared_data.linkSkillList[num]["skills"]:
        for _ in range(i[1]):
            taskList.append(
                [
                    convert7to5(shared_data, i[0]),
                    shared_data.IS_SKILL_CASTING[shared_data.serverID][shared_data.jobID][i[0]],
                ]
            )
...
def addTaskList(shared_data):
    # append
    lCopy = copy.deepcopy(shared_data.preparedSkillList)
    for skill in lCopy[2]:
        if shared_data.ifUseSkill[shared_data.selectedSkillList[skill]]:
            if skill in lCopy[1]:
    ...
    # 연계스킬 사용
    while len(shared_data.preparedLinkSkillList) != 0:
        for j in range(len(shared_data.usingLinkSkillList[shared_data.preparedLinkSkillList[0]])):
            skill = shared_data.usingLinkSkillList[shared_data.preparedLinkSkillList[0]][j]
            count = shared_data.usingLinkSkillComboList[shared_data.preparedLinkSkillList[0]][j]
                    ...
                    for idx in range(len(shared_data.preparedSkillList[k])):
                        if skill == shared_data.preparedSkillList[k][idx]:
                            for _ in range(count):
                                if shared_data.IS_SKILL_CASTING[shared_data.serverID][shared_data.jobID][
                                    shared_data.selectedSkillList[skill]
                                ]:
                                    shared_data.taskList.append([skill, True])  # skill: k, click: True
                                else:
                                    shared_data.taskList.append([skill, False])  # skill: k, click: False
                                ...
                                # )
        checkIsLinkReady(shared_data)
    shared_data.preparedLinkSkillList = []
    reloadPreparedSkillList(shared_data)
    ...
    for i in shared_data.skillSequences:
        ...
        if (
            (i in shared_data.preparedSkillList[0] or i in shared_data.preparedSkillList[1])
            and i in tempL
            and shared_data.ifUseSole[shared_data.selectedSkillList[i]]
        ):
            for x in [0, 1]:
                for j, k in enumerate(shared_data.preparedSkillList[x]):
                    if i == k:
                        while shared_data.preparedSkillCountList[x][j] >= 1:
                            if shared_data.IS_SKILL_CASTING[shared_data.serverID][shared_data.jobID][
                                shared_data.selectedSkillList[k]
                            ]:
                                shared_data.taskList.append([k, True])  # skill: k, click: True
                            else:
                                shared_data.taskList.append([k, False])  # skill: k, click: False
        ...
        if (
            ...
            and not (i in tempL)
            and shared_data.ifUseSkill[shared_data.selectedSkillList[i]]
        ):
            for j, k in enumerate(shared_data.preparedSkillList[1]):
                if i == k:
                    while shared_data.preparedSkillCountList[1][j] >= shared_data.preparedSkillComboList[i]:
...
def checkIsLinkReady(shared_data):
    shared_data.preparedLinkSkillList = []
    for x in range(len(shared_data.linkSkillRequirementList)):
        ...
        if not False in ready:
            shared_data.preparedLinkSkillList.append(x)


def reloadPreparedSkillList(shared_data):
    # self.printMacroInfo()
    # print("\n")
...
def printMacroInfo(shared_data, brief=False):
    if brief:
        print("테스크 리스트:", shared_data.taskList)  # 사용여부 x, 사용여부 o
        print("준비된 스킬 리스트:", shared_data.preparedSkillList)  # 사용여부 x, 사용여부 o
        print("준비된 스킬 개수 리스트:", shared_data.preparedSkillCountList)  # 사용여부 x, 사용여부 o
        print("준비된 연계스킬리스트:", shared_data.preparedLinkSkillList)
    else:
        print("준비된 스킬 리스트:", shared_data.preparedSkillList)  # 사용여부 x, 사용여부 o
        print("준비된 스킬 개수 리스트:", shared_data.preparedSkillCountList)  # 사용여부 x, 사용여부 o
        print("스킬 콤보 리스트:", shared_data.preparedSkillComboList)  # 사용여부 o
        print("스킬 정렬 순서:", shared_data.skillSequences)
        print("연계스킬 스킬 리스트:", shared_data.usingLinkSkillList)
