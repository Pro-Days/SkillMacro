...
from app.scripts.app_state import MacroState, UiState, app_state
...
def reset_app_state() -> Iterator[None]:
    """전역 AppState 싱글톤을 테스트마다 초기 상태로 재구성"""

    # 다른 모듈들이 app_state 객체 참조를 직접 보유하므로 교체가 아닌 필드 재할당으로 초기화
    app_state.macro = MacroState()
    app_state.ui = UiState()
...
def isolated_data_paths(
    ...
    """data_manager의 저장 경로를 임시 디렉터리로 격리"""

    # 사용자 환경의 실제 %LOCALAPPDATA% 데이터를 읽거나 쓰지 않도록 모듈 전역 경로 우회
    fake_data_path: str = str(tmp_path)
    fake_file_dir: str = str(tmp_path / "macros.json")
    fake_custom_skills_dir: str = str(tmp_path / "custom_skills.json")
    fake_characters_dir: str = str(tmp_path / "characters.json")

    monkeypatch.setattr(data_manager, "data_path", fake_data_path)
    monkeypatch.setattr(data_manager, "file_dir", fake_file_dir)
    monkeypatch.setattr(data_manager, "custom_skills_file_dir", fake_custom_skills_dir)
    monkeypatch.setattr(data_manager, "characters_file_dir", fake_characters_dir)

    return {
        "data_path": fake_data_path,
        "file_dir": fake_file_dir,
        "custom_skills_file_dir": fake_custom_skills_dir,
        "characters_file_dir": fake_characters_dir,
    }
...
def build_synthetic_server(
    ...
    # 무공비급/스킬 ID 규칙: scroll_<i> 무공비급에 [scroll_<i>_a, scroll_<i>_b]
...
def build_full_equipped_preset(
    ...
    전체 스킬이 우선순위 0, use_skill=True 기본 상태로 배치된다.
    """

    scroll_ids: list[str] = server_spec.skill_registry.get_all_scroll_ids()
    if len(scroll_ids) != server_spec.scroll_slot_count:
        raise ValueError("synthetic server scroll count must match scroll_slot_count")

    # 무공비급 순서를 슬롯에 그대로 배치
    equipped_scrolls: list[str] = list(scroll_ids)

    # 하단 배치칸에 각 무공비급의 두 스킬을 line_index 0/1로 배치
    ...
    skill_keys: list[str] = [str(2 + i) for i in range(server_spec.scroll_slot_count)]
...
def make_calculator_input(
    ...
    return CalculatorPresetInput(
        ...
        target_danjeon=TargetDanjeonState(),
        candidate_groups=[],
...
def make_realistic_base_stats() -> BaseStats:
    ...
    values: dict[str, float] = {stat_key.value: 0.0 for stat_key in OVERALL_STAT_ORDER}
